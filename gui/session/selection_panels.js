...
g_SelectionPanels.Upgrade = {
	...
	"getItems": function(unitEntStates)
	{
		const upgrades = unitEntStates[0].upgrade && unitEntStates[0].upgrade.upgrades;
		if (!upgrades)
			return false;

		// Empires Ignited: stock hides this panel for any mixed-template
		// selection. Workers come in several templates (female citizen,
		// civilian, house-trained variants) that all morph to the same unit,
		// so allow a mixed selection as long as every selected unit offers the
		// exact same set of upgrade targets.
		const key = upgrades.map(u => u.entity).sort().join(",");
		if (unitEntStates.some(state =>
			!state.upgrade || !state.upgrade.upgrades ||
			state.upgrade.upgrades.map(u => u.entity).sort().join(",") != key))
			return false;
