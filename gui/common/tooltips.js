...
/**
 * Empires Ignited: render attack bonuses (multiplier vs target classes),
 * e.g. "×3 vs Structure". Stock 0 A.D. does not show these anywhere.
 */
function attackBonusDetails(bonuses)
{
	if (!bonuses)
		return "";
	...
	{
		const multiplier = +bonuses[name].multiplier;
		if (!multiplier || multiplier == 1)
			continue;
		details.push(sprintf(translate("×%(multiplier)s vs %(classes)s"), {
			"multiplier": multiplier % 1 ? +multiplier.toFixed(2) : multiplier,
			"classes": String(bonuses[name].classes).split(/\s+/).filter(c => c).join(", ")
		}));
	}
	return details.join(commaFont(translate(", ")));
}
...
function attackEffectsDetails(attackTypeTemplate)
{
	...
	const effects = [
		...
		attackBonusDetails(attackTypeTemplate.Bonuses || undefined)
	];
