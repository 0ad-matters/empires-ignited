...
function GetTemplateDataHelper(template, player, auraTemplates, resources, modifiers = {})
{
	...
	const getAttackEffects = (temp, path) => {
		...
		// Empires Ignited: expose attack bonuses (multiplier vs target classes)
		// so the GUI can show them. Multiplier read via getEntityValue so it
		// reflects upgrades on a selected entity (base value in previews).
		if (temp.Bonuses)
		{
			effects.Bonuses = {};
			for (const bonusName in temp.Bonuses)
				effects.Bonuses[bonusName] = {
					"classes": temp.Bonuses[bonusName].Classes &&
						(temp.Bonuses[bonusName].Classes._string || temp.Bonuses[bonusName].Classes) || "",
					"multiplier": getEntityValue(path + "/Bonuses/" + bonusName + "/Multiplier")
				};
		}
